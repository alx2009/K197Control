...
/*!
  ...
  The GeminiK197Control class implements the application layer of the K197
  control protocol. Control frames are sent to/measurement results received from
  the K197 using the gemini frame layer (via base class geminiFrame)
...
/*!
      @brief handles communications with a K197 voltmeter using the interface
   used by the IEEE-488 option card

      @details the K197 can be equipped with an optional IEEE-488 card. This
   card communicates with the K197 main board with a 2 wire interface. This
   class implements the application layer of this 2 wire interface.

      The class uses the gemini protocol inherithed by the base classes to
   receive measurement results and send control commands to the K197.

      After construction, begin() must be called before using any other
   function. Then update() must be called on a regular basis.

      When a measurement is received, it is stored in a K197measurement buffer.
   FrameComplete() in the base class can be used to know when a new frame has
   been received. One of the methods resetFrame() or getFrame() in the base
   class must be called before a new frame can be received.

      To send control commands two methods can be used:
      - execute() queues the commands currently stored in the current control
   structure to be sent as soon as possible as a new frame. This is the
   recommended (and safer) way to control the k197. The current control buffer
   is the one specified at begin() or using setControlBuffer
      - sendImmediately() will queue the specified control buffer structure
   immediately. With this method the caller must make sure that there isn't a
   transmission already queued or ongoing, otherwise the effect can be
   unpredictable. It is recommended that the caller make sure both
   isFrameEndDetected() and noOutputPending() return true before calling
   sendImmediately().
*/
...
public:
  /*!
     ...
     outpout pin to LOW
  */
  GeminiK197Control(uint8_t inputPin, uint8_t outputPin,
  ...
  */
  ...
  /*!
      ...
      When controlling the K197, only Volt and Ampere can be changed
     programmatically When reading the measurement results, the range reflects
     the actual measurement range (auto range is never returned) See the
     instruction manual for the K197 IEEE-488 for more information
  */
  enum K197range {
  ...
  /*!
      ...
      @details this is a four byte data structure to access the measurement
     results from the K197.
     All data can be accessed via the data members, but using the
     provided methods is probably more convenient
  */
  struct K197measurement {
    ...
    */
    union {
      uint8_t
          frameBuffer[1]; ///< allows access to all the data as uint8_t array
      ...
      int32_t value; ///< allows access to all bytes as a 32 bit signed integer
                     ///< value
      uint32_t uvalue; ///< allows access to all bytes as a 32 bit unsigned
                       ///< integer value
    ...
    /*!
       @brief check zero
       @return true if the measured value is zero
    */
    inline bool isZero() const {
      return lsb.lo == 0 && lsb.hi == 0 && byte1.msb == 0 ? true : false;
    };
    /*!
       @brief check AC
       @return true if AC measurement
    */
    inline bool isAC() const { return byte0.ac_dc; };
    /*!
       @brief check DC
       @return true if DC measurement
    */
    inline bool isDC() const { return !byte0.ac_dc; };
    /*!
       @brief check relative 
       @return true if relative measurement
    */
    inline bool isRelative() const { return byte0.relative; };
    /*!
       @brief check absolute 
       @return true if absolute measurement
    */
    inline bool isAbsolute() const { return !byte0.relative; };

    /*!
       @brief check Volt 
       @return true if the measurement unit is Volt
    */
    inline bool isVolt() const {
      return byte0.unit == K197unit::Volt ? true : false;
    };
    /*!
       @brief check Ohm 
       @return true if the measurement unit is Ohm
    */
    inline bool isOhm() const {
      return byte0.unit == K197unit::Ohm ? true : false;
    };
    /*!
       @brief check Ampere
       @return true if the measurement unit is Ampere
    */
    inline bool isAmp() const {
      return byte0.unit == K197unit::Amp ? true : false;
    };
    /*!
       @brief check dB
       @return true if the measurement unit is dB
    */
    inline bool is_dB() const {
      return byte0.unit == K197unit::dB ? true : false;
    };

    /*!
       @brief check negative
       @return true if the measurement value sign is negative
    */
    inline bool isNegative() const { return byte1.negative; };
    /*!
       @brief check over-range condition
       @return true if the measurement exceed the measurement range (over-range conditon)
    */
    inline bool isOvrange() const { return byte1.ovrange; };

    /*!
       @brief get the binary count
       @return binary count (no sign), ranges from 0x000000 to x1FFFFF,
       corresponding to the K197 display visualizing 0 to 400000
    */
    unsigned long getCount() const {
      return (((int32_t)byte1.msb) << 16) + (((int32_t)lsb.hi) << 8) +
             (((int32_t)lsb.lo));
    };
  ...
  */
  ...
  /*!
      ...
      @details we define a named value for each of the trigger mode mentioned in
     the instruction manual for the K197 IEEE-488 However, there is no
     difference between TALK and GET on the internal interface. See the
     instruction manual for the K197 IEEE-488 for more information In addition,
     we define other constants that are used to actually trigger the measure
  ...
  */
  enum K197triggerMode {
    ...
    // the following bitmaps can be or-ed together to define the required mode
    // combination
  ...
  */
  ...
  /*!
      ...
      Any data can be accessed via the data members, but using the
        provided methods is probably more convenient
      @return Not really a return type, this attribute will save some RAM
  */
  struct K197control {
    ...
    */
    ...
    /*!
        ...
        @details Note that sending an empty request to the K197 will not do anything
...
/*!
     ...
     @param isAbsolute true (default) set absolute mode. False set relative mode
*/
    ...
    /*!
        @brief  set local or remote mode
        @param isLocal true (default) set local mode. False set remote mode
    */
    ...
    /*!
        @brief  set displayed or stored reading mode
       @param sendDisplay true (default) set displayed reading mode. False set stored reading mode
    */
    void setSendDisplayReadings(bool sendDisplay = true) {
      setSendStoredReadings(!sendDisplay);
    };
  };
...
public:
  ...
  /*!
      ...
      @details in this function we update the internal state machine, reading
     and writing data as required by the K197
  ...
  */
  K197measurement *getMeasurementBuffer() const { return inputBuffer; };
  /*!
      @brief set the measurement buffer
      @details the new buffer will receive the result of the measurements from
     the K197
      @param newInputBuffer a pointer to a meaasurement buffer
      @param resetBuffer if true the buffer is reset (same as calling
     newInputBuffer.clear()). Default is false
  */
  void setMeasurementBuffer(K197measurement *newInputBuffer,
                            bool resetBuffer = false) {
    inputBuffer = newInputBuffer;
    setInputBuffer((uint8_t *)newInputBuffer,
                   sizeof(K197measurement) / sizeof(uint8_t), resetBuffer);
  };
  ...
  */
  K197control *getControlBuffer() const { return outputBuffer; };
  /*!
      @brief set the control buffer
      @details the new buffer will be used by execute() and the variant of
     sendImmediately() without a pointer to a K197control structure
      @param newOutputBuffer a pointer to a meaasurement buffer
      @param resetBuffer if true the buffer is reset (same as calling
     newInputBuffer.clear()). Default is false
  */
  void setControlBuffer(K197control *newOutputBuffer,
                        bool resetBuffer = false) {
  ...
  /*!
      ...
      When using this method, the caller must make sure that there isn't a
     transmission already queued or ongoing, otherwise the effect can be
     unpredictable. It is recommended that the caller make sure both
     isFrameEndDetected() and noOutputPending() return true before calling
     sendImmediately().

      @param bufferToSend a pointer to the meaasurement buffer that should be
     sent
      @param resetAfterSending if true the buffer is reset after sending (same
     as calling bufferToSend.clear()). Default is true
      @return true if the buffer could be sent, false otherwise
  */
  bool sendImmediately(K197control *bufferToSend,
                       bool resetAfterSending = true) {
    if (bufferToSend == NULL)
      return false;
    sendFrame((uint8_t *)bufferToSend, sizeof(K197control) / sizeof(uint8_t));
    if (resetAfterSending)
      bufferToSend->clear();
    return true;
  };
  ...
  /*!
      ...
      @details When using this method, the caller must make sure that there
     isn't a transmission already queued or ongoing, otherwise the effect can be
     unpredictable. It is recommended that the caller make sure both
     isFrameEndDetected() and noOutputPending() return true before calling
     sendImmediately().

      @param resetAfterSending if true the buffer is reset after sending (same
     as calling getControlBuffer()->clear()). Default is true
      @return true if the buffer could be sent, false otherwise
 */
  ...
  /*!
      ...
      @details This is the safest and recommended method to send a control frame
     to the K197. When using this method, an internal flag is set to request
     transmission of the control frame. When this flag is set, update() will
     make sure the control buffer is sent to the lower layer (see
     GeminiFrame::sendFrame()), making sure only one control buffer at a time
     can be transmitted.

      The current input buffer can be modified at any time until update() send
     the buffer with GeminiFrame::sendFrame(). The information sent will reflect
     all the modifications until that point.

      If the sender want to know when update() has sent the buffer, it can
     checked executeComplete(). When executeComplete() returns true, any change
     to the control buffer will be sent as as eparate frame.

      Note that executeComplete() does not esnure that the information has been
     actually sent to the K197. this is up to the lower layers. In order to know
     that a sert of command has been sent to the K197, the following steps are
     recommended (see also note below):
      ...
      Note: the application should continue to call update() between the steps
     above, otherwise the protocol will hang.
  */
  void execute() {
...
private:
  K197measurement *inputBuffer = NULL; ///< stored received measurement results
  K197control *outputBuffer = NULL;    ///< store control commands to be sent

protected:
  using GeminiFrame::begin;
  using GeminiFrame::sendFrame;
  bool outputQueued =
      false; ///< flag that outputBuffer shall be sent as soon as possible
};
